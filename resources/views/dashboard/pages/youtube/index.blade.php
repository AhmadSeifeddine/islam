...
@section('toolbar')
    @include('dashboard.components.toolbar', [
        'title' => 'Youtube',
        'currentPage' => 'Youtube Management',
    ])
@endsection
...
@php
    $columns = ['title', 'url', 'scholar_id', 'category_id', 'duration', 'status', 'actions'];
@endphp
...
---------------------------->
@section('content')
    <x-lebify-table id="youtubeTable" :columns="$columns" {{-- create="true"                         // BY DEFAULT TRUE
...
---------------------------->
@push('scripts')
<script src="{{ asset('js/dashboard/youtube.js') }}" type="module" defer></script>
@endpush
