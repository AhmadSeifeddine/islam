            ...
            <div class="datatable-body mb-5">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    @if ($showSearch ?? true)
                        ...
                        <input type="text" data-table-filter="search"
                            ...
                            placeholder="{{ __('common.dashboard.search_placeholder') }}" />
                    @endif
                    <!-- Column Visibility -->
                            ...
                            <div class="menu menu-sub menu-sub-dropdown w-250px w-md-300px" data-kt-menu="true">
                                <div class="px-7 py-5">
                                    <div class="fs-5 fw-bolder">
                                        {{ $columnSettingsTitle ?? __('common.dashboard.column_settings') }}</div>
                                </div>
                ...
                <div class="d-flex filter-toolbar flex-wrap align-items-center gap-2 gap-lg-3">
                        ...
                        <div class="menu menu-sub menu-sub-dropdown w-250px w-md-300px" data-kt-menu="true"
                            ...
                            <div class="px-7 py-5">
                                <div class="fs-5 text-gray-900 fw-bold">
                                    {{ $filterTitle ?? __('common.dashboard.filter_options') }}</div>
                            </div>
                            <div class="separator border-gray-200"></div>
                            <div class="px-7 py-5">
                                <div class="mb-10">
                                ...
                                <div class="d-flex justify-content-end">
                                    <button type="reset" class="btn btn-sm btn-light btn-active-light-primary me-2"
                                        ...
                                        {{ $resetButtonText ?? __('common.dashboard.reset') }}
                                    </button>
                                    <button type="submit" class="btn btn-sm btn-primary" data-kt-menu-dismiss="true"
                                        data-table-filter="filter">
                                        {{ $applyButtonText ?? __('common.dashboard.apply') }}
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    @if (isset($create) && $create)
                        <a href="{{ $createRoute ?? '#' }}"
                            class="btn btn-sm create fw-bold {{ $createButtonClass ?? 'btn-primary' }}"
                            @if ($createRoute ?? false) @else
                           data-bs-toggle="modal" data-bs-target="#{{ $createModalId ?? 'create-modal' }}" @endif>
                            {{ $createButtonText ?? __('common.dashboard.create') }}
                        </a>
                    @endif
