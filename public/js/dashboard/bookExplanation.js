/*=============================================================================
 * Book_Explanation Management Module
 *
 * This module handles all book_Explanation-related operations in the dashboard including:
 ...
 * @function buildApiUrl - Constructs API endpoints for book_Explanation operations
 *--------------------------------------------------------------------------*/
const defaultErrorHandler = (err) => console.error('Error:', err);
const reloadDataTable = () => bookExplanationTable.reload();
const buildApiUrl = (path) => `${DASHBOARD_URL}/books/explanations/${path}`;
...
const userActionHandlers = {
    ...
    edit: function (id) {
        this.callCustomFunction('_EDIT_', id, buildApiUrl(`${id}/edit`), (response) => {
            initSelect2("#book_select", "#edit-modal");
            initSelect2("#youtube_select", "#edit-modal");
        });
    },
 ...
 * Defines the structure and behavior of the Book_Explanation management table
 *--------------------------------------------------------------------------*/
const tableColumns = [
    {
        "data": "id"
...
const tableColumnDefinitions = [
    ...
    { targets: [3], orderable: false, htmlType: 'toggle', dataClassName: 'btn-status' },
    {
        targets: [-1],
        htmlType: 'actions',
        className: 'text-end',
...
/*---------------------------------------------------------------------------
 * Bulk Action Handler
 * Processes operations on multiple selected book_Explanations
 * @param {Array} selectedIds - Array of selected book_Explanation IDs
 *--------------------------------------------------------------------------*/
const handleBulkActions = (selectedIds) => {
    // Implementation for bulk actions
    // Example: Delete multiple book_Explanations, change status, etc.
};

/*=============================================================================
 * DataTable Initialization
 * Creates and configures the main book_Explanation management interface
 *============================================================================*/
export const bookExplanationTable = new $DatatableController('bookExplanationTable', {
...
// Initialize create book_Explanation modal
createModalLoader({
    triggerSelector: '.create',
    endpoint: buildApiUrl('create'),
    onSuccess: () => {
        initSelect2("#book_select", "#create-modal");
        initSelect2("#youtube_select", "#create-modal");
    }
});
